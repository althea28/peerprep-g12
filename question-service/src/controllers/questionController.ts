...
export async function getAllQuestions(req: Request, res: Response) {
    ...
    if (!withBlocks) return res.status(500).json( { error: 'Failed to fetch question blocks.'});
...
export async function createQuestion(req: Request, res: Response) {
    ...
    const { error : blockInsertError } = await insertBlocks(newQuestion.id, blocks);
    if (blockInsertError) return res.status(500).json({ error: blockInsertError.message });
...
export async function editQuestion(req: Request, res: Response) {
    ...
    if (blockFetchError) return res.status(500).json({ error: blockFetchError.message });
    ...
    const blocksChanged = 
        JSON.stringify(currentBlocks?.map(({ block_type, content }: any) => ({ block_type, content }))) !==
        JSON.stringify(incomingBlocks?.map(({ block_type, content }: any) => ({ block_type, content })));
    ...
    const blocksToInsert = blocks ?? currentBlocks!.map(({ block_type, content }: any) => ({ block_type, content}));
    const { error: blockInsertError } = await insertBlocks(newVersion.id, blocksToInsert);
    if (blockInsertError) return res.status(500).json({ error: blockInsertError.message });
